...
const DashboardMesero = () => {
  ...
  // Obtiene el token o redirige si no existe
  const getToken = () => {
  ...
  // Manejador centralizado de errores de fetch
  ...
  const cancelarPedido = async () => {
    // AÑADIDO: Obtener token y verificar
    const token = getToken();
    if (!token || !pedidoParaCancelar) return;

    try {
      setError(""); // AÑADIDO: Limpiar error
...
      );
      ...
      // AÑADIDO: Manejo de 401 y errores
      if (response.status === 401) throw new Error("Unauthorized");
      if (!response.ok) {
        const data = await response.json(); // Intentar leer el error
...
      await fetchPedidos();
    } catch (error) {
      // MODIFICADO: Usar el manejador de errores
      handleFetchError(error, "Error al cancelar el pedido");
      setShowConfirmCancelarModal(false); // AÑADIDO: Cerrar modal si hay error
    }
  };
...
      const ahora = new Date();
      // Pedidos activos: Pendiente o Preparando
      const pedidosFiltrados = data.filter((pedido) => {
        return pedido.estado !== "COBRADO" && pedido.estado !== "ENTREGADO";
      });

      // Pedidos entregados (para la otra vista)
      const entregados = data.filter((pedido) => {
        if (pedido.estado === "ENTREGADO") {
          // Usar 'modificado_en' si está disponible, sino 'hora_pedido'
          const tiempoReferencia = pedido.modificado_en || pedido.hora_pedido;
          const tiempoEntrega = new Date(tiempoReferencia);
          const diferenciaTiempo = (ahora - tiempoEntrega) / 1000 / 60; // diferencia en minutos
          return diferenciaTiempo > 3; // Mostrar si pasaron más de 3 minutos
        }
        return false;
      });
...
      handleFetchError(error, "Error al cargar pedidos");
      setPedidos([]);
...
      if (!response.ok) throw new Error(data.error || "Error al crear pedido");
      ...
      // MODIFICADO: Usar el manejador de errores
      handleFetchError(error, `Error al enviar el pedido: ${error.message}`);
    }
  };
...
  const marcarComoEntregado = async () => {
    // AÑADIDO: Obtener token y verificar
    const token = getToken();
    if (!token || !pedidoParaEntregar) return;
...
      if (!response.ok) {
        const data = await response.json(); // Intentar leer el error
        throw new Error(data.error || "Error al marcar pedido como entregado");
      }
...
    } catch (error) {
      // MODIFICADO: Usar el manejador de errores
      handleFetchError(error, "Error al marcar como entregado");
      setShowConfirmEntregadoModal(false); // AÑADIDO: Cerrar modal si hay error
    }
  };
